...
### Final Deliverable
A POST request to `https://***hub_endpoint***/verify` with:
```json
{
...
**Behavior:**
- Send a POST request to `https://***hub_endpoint***/verify` with JSON body:
  ```json
  {
...
START
  ...
  │     Documentation entry URL: https://***hub_endpoint***/dane/doc/index.md
  │
...
- **Link discovery:** The agent must parse fetched markdown/HTML for relative and absolute links and resolve them against the base URL (`https://***hub_endpoint***/dane/doc/`).
...
- [ ] **Successful submission:** The agent submits the declaration to `https://***hub_endpoint***/verify` and receives a `{FLG:...}` response.
