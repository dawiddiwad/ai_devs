...
14. `api-client.ts`: Axios wrapper for POST to `https://***hub_endpoint***/api/packages` with two functions:
...
21. Submit to hub: POST to `https://***hub_endpoint***/verify` with `{ apikey, task: "proxy", answer: { url: "<tunnel-url>", sessionID: "test-session-01" } }`
...
- `src/api-client.ts` — Axios calls to ***hub_endpoint*** packages API + verify endpoint
...
7. **Final submission**: POST to ***hub_endpoint***/verify and check response for success
