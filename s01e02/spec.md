...
1. **Lista elektrowni + ich kody**
   - Pobierz JSON z listą elektrowni (wraz z kodami identyfikacyjnymi) z:
     - `https://***hub_endpoint***/data/AI_DEVS_API_KEY/findhim_locations.json`

2. **Gdzie widziano konkretną osobę (lokalizacje)**

   - Endpoint: `https://***hub_endpoint***/api/location`
   - Metoda: `POST`
...
3. **Jaki poziom dostępu ma wskazana osoba**
   ...
   - Endpoint: `https://***hub_endpoint***/api/accesslevel`
   - Metoda: `POST`
   - Body: `raw JSON`
...
Wysyłasz ją metodą **POST** na `https://***hub_endpoint***/verify`.
